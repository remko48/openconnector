...
class AuthenticationService
{
    ...
    private function createClientCredentialConfig(array $configuration): array
    {
        ...
        if (isset($configuration['client_assertion_type']) && $configuration['client_assertion_type'] === 'urn:ietf:params:oauth:client-assertion-type:jwt-bearer') {
    ...
    public function fetchOAuthTokens(array $configuration): string
    {
        ...
        if (isset($configuration['tokenLocation'])) {
    ...
    private function getHSJWK(array $configuration): ?JWK
    {
        return new JWK(
            [
                'kty' => 'oct',
                'k' => rtrim(base64_encode(addslashes($configuration['secret'])), '='),
            ]
        );
    }
    ...
    {
        $jwk = null;
        if (in_array($configuration['algorithm'], ['HS256', 'HS512'])) {
            return $this->getHSJWK($configuration);
        } else if (in_array($configuration['algorithm'], ['RS256', 'RS384', 'RS512', 'PS256'])) {
    ...
    public function fetchJWTToken(array $configuration): string
    {
        ...
        if (isset($configuration['x5t'])) {
            return $this->generateJWT($payload, $jwk, $configuration['algorithm'], $configuration['x5t']);
        }
