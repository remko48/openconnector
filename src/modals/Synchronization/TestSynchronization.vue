...
<template>
	<NcModal v-if="navigationStore.modal === 'testSynchronization'"
		...
		<div class="modalContent">
			...
			<NcNoteCard v-if="response && response.status.toString().startsWith('2')" type="success">
				<p>The connection to the synchronization was successful.</p>
			</NcNoteCard>
			<NcNoteCard v-if="(response && !response.status.toString().startsWith('2')) || error" type="error">
				<p>
